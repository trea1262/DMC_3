...
def parse_args():
    ...
    parser.add_argument('--dataset_split_type', type=str, default='indirect', help='direct/indirect')
    parser.add_argument('--num_frames_per_video', type=int, default=16)
    parser.add_argument('--frame_resolution', type=int, default=224)
    parser.add_argument('--model_name', type=str, default='.../pretrain_model/EgoVLPv2.pth', help="Path of the pretrained checkpoint")
    parser.add_argument('--resume_finetune_model_path', type=str, default='', help ="Path of the previously fine-tuned checkpoint for resume")#default='.../pretrain_model/EgoTaskQA_finetune_indirect.tar',
    parser.add_argument('--test_only_model_path', type=str,  help ="Path of the final fine-tuned checkpoint only for evaluation")
...
def main_worker(gpu, args):
    ...
    torch.distributed.init_process_group(
        backend='gloo', init_method=args.dist_url,#nccl
    ...
    dataset_train = EgoTaskQA(dataset_name="EgoTaskQA", text_params={"input": "text"},
                                  video_params={"input_res": args.frame_resolution,
                                                ...
                                                "loading": "lax"},
                                  data_dir=".../data/qa_videos",
                                  meta_dir=".../data/Data/qa_ori/" + args.dataset_split_type,
    ...
    dataloader_test = DataLoader(dataset_test, batch_size=args.per_gpu_batch_size, drop_last=True, sampler=sampler_test, collate_fn=collate_func)
    ...
    with open('.../data/Data/qa_ori/' + args.dataset_split_type + '/answer_set.txt', 'r') as ansf:
        answers = ansf.readlines()
        args.output_dim = len(answers) # # output_dim == len(answers) 961#
    ...
    if args.resume_finetune_model_path is not None:
        ...
        model.load_state_dict(new_ckpt_model,False)
    ...
    if args.test_only_model_path is not None:
        ...
        new_ckpt_model = state_dict_data_parallel_fix(ckpt['model_state_dict'], model.state_dict())
        model.load_state_dict({k.replace('module.',''):v for k,v in new_ckpt_model.items()}, False)
    ...
    with open('.../data/Data/qa_ori/' + args.dataset_split_type + '/all_reasoning_types.txt', 'r') as reasonf:
    ...
    unique_dict = torch.load('.../pretrain_model/reasoning_unique_cat.pth')
    ...
    for epoch in range(start_epoch, args.nepoch):
        ...
        for idx, (video_frames, mask_video_frames, video_center_frames, text_tokens, attention_masks,textneg_tokens, attentionneg_masks, textpos_tokens, attentionpos_masks, answers, reasoning_type_lst) in tqdm(enumerate(dataloader_train)):
            ...
            mask_video_frames, video_center_frames = mask_video_frames.cuda(gpu, non_blocking=True), video_center_frames.cuda(gpu, non_blocking=True)
            textneg_tokens, attentionneg_masks, textpos_tokens, attentionpos_masks = textneg_tokens.cuda(gpu, non_blocking=True), attentionneg_masks.cuda(gpu, non_blocking=True), textpos_tokens.cuda(gpu, non_blocking=True), attentionpos_masks.cuda(gpu, non_blocking=True)
            data_batch = {"video": video_frames, "video_positive": mask_video_frames, "video_negative": video_center_frames, "text_neg": {"input_ids": textneg_tokens, "attention_mask": attentionneg_masks},"text_pos": {"input_ids": textpos_tokens, "attention_mask": attentionpos_masks},"text": {"input_ids": text_tokens, "attention_mask": attention_masks}, "answer":answers}
            
            with torch.set_grad_enabled(True):
                with torch.cuda.amp.autocast():
                    
                    logits,logits1,logits2,loss = model.forward(data_batch)
                    logits = args.allgather(logits, gpu, args)
                    answers = args.allgather(answers, gpu, args)
                    loss = criterion(logits, answers.long())+criterion(logits1, answers.long())+criterion(logits2, answers.long())+0.01*loss
...
def validate(model, val_dataloader, epoch, args, gpu, acc_calculator):
    ...
    with torch.no_grad():
        starttime = time.time()
        for idx, (video_frames, mask_video_frames, video_center_frames, text_tokens, attention_masks,textneg_tokens, attentionneg_masks, textpos_tokens, attentionpos_masks, answers, reasoning_type_lst) in tqdm(enumerate(val_dataloader)):
            B, num_frame_per_video, C, H, W = video_frames.shape
            video_frames, mask_video_frames, video_center_frames, text_tokens, attention_masks, answers = video_frames.cuda(gpu, non_blocking=True), mask_video_frames.cuda(gpu, non_blocking=True), video_center_frames.cuda(gpu, non_blocking=True), text_tokens.cuda(gpu, non_blocking=True), attention_masks.cuda(gpu, non_blocking=True), answers.cuda(gpu, non_blocking=True)
            textneg_tokens, attentionneg_masks, textpos_tokens, attentionpos_masks = textneg_tokens.cuda(gpu, non_blocking=True), attentionneg_masks.cuda(gpu, non_blocking=True), textpos_tokens.cuda(gpu, non_blocking=True), attentionpos_masks.cuda(gpu, non_blocking=True)
            
            data_batch = {"video": video_frames, "video_positive": mask_video_frames, "video_negative": video_center_frames,"text_neg": {"input_ids": textneg_tokens, "attention_mask": attentionneg_masks},"text_pos": {"input_ids": textpos_tokens, "attention_mask": attentionpos_masks},"text": {"input_ids": text_tokens, "attention_mask": attention_masks}, "answer":answers}
            logits,_,_,_ = model(data_batch)
...
def main():
    ...
    if 'SLURM_JOB_ID' in os.environ:
        ...
        args.dist_url = f'tcp://{host_name}:52366'
    else:
        # single-node distributed training
        args.rank = 0
        args.dist_url = 'tcp://localhost:52366'
