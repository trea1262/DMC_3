...
class EgoTaskQA(TextVideoDataset):
    def _load_metadata(self, args):
        metadata_dir = '...' + args.dataset_split_type
        split_files = {
            'train': 'formatted_train_qas_encode1.json',
            'val': 'formatted_val_qas_encode1.json',           
            'test': 'formatted_test_qas_encode1.json',  
        }
...
def collate_func(batch):
    ...
    config = '.../roberta-base/config.json'
    unique_dict = torch.load('.../reasoning_unique_cat.pth')
    f = open(config)
    config = json.load(f)
    tokenizer = transformers.AutoTokenizer.from_pretrained(".../roberta-base/", local_files_only=True)
