...
def vis_detections_filtered_objects_(im, obj_dets, hand_dets, thresh_hand=0.8, thresh_obj=0.01, font_path='/data/zoujy/hand_object_detector-master/lib/model/utils/times_b.ttf'):
    # Convert to PIL
    im = im[:, :, ::-1]
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for obj_idx, i in enumerate(range(np.minimum(10, obj_dets.shape[0]))):
            ...
            if score > thresh_obj and i in img_obj_id:
                ...
                # Update mask1 for object detection
                x1, y1, x2, y2 = bbox
        ...
        for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
            ...
            if score > thresh_hand:
                ...
                # Update mask1 for hand detection
                x1, y1, x2, y2 = bbox
                mask1[y1:y2, x1:x2] = 255
    ...
    # Create mask2 as the inverse of mask1
    mask2 = 255 - mask1

    return image, mask1, mask2

def vis_detections_filtered_objects_PIL(im, obj_dets, hand_dets, thresh_hand=0.8, thresh_obj=0.01, font_path='/data/zoujy/hand_object_detector-master/lib/model/utils/times_b.ttf'):
